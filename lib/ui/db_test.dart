...
Future<void> main() async {
  ...
  try {
    ...
    try {
      var deptCount = await db.rawQuery('SELECT COUNT(*) as count FROM department');
      print('  Department: ${deptCount.first['count']}');
    } catch (e) { print('  Department: 0'); }
    
    try {
      var doctorCount = await db.rawQuery('SELECT COUNT(*) as count FROM doctor');
      print('  Doctor: ${doctorCount.first['count']}');
    } catch (e) { print('  Doctor: 0'); }
    
    try {
      var staffCount = await db.rawQuery('SELECT COUNT(*) as count FROM staff');
      print('  Staff: ${staffCount.first['count']}');
    } catch (e) { print('  Staff: 0'); }
    
    try {
      var patientCount = await db.rawQuery('SELECT COUNT(*) as count FROM patient');
      print('  Patient: ${patientCount.first['count']}');
    } catch (e) { print('  Patient: 0'); }
    
    try {
      var appointmentCount = await db.rawQuery('SELECT COUNT(*) as count FROM appointment');
      print('  Appointment: ${appointmentCount.first['count']}');
    } catch (e) { print('  Appointment: 0'); }
    ...
    if (appointments.isEmpty) {
      ...
      for (var a in appointments.take(5)) {
        print('  ID: ${a['id']} | ${a['patient_name']} → Dr. ${a['doctor_name']} (${a['date']})');
      }
